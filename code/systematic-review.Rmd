...
```
  ...
  * Revise and then send for consensus
  ...
  [ ] Paper by two coders, but have not gone through `Consensus`
...
dt.bymhi.cl2 <- dt.bymhi.cl  %>%
  ...
  dplyr::mutate(
    dimension = factor(dimension, 
                       levels = c('Type', 'type_top', 'Levels'), 
                       labels = c('Flow examples', 'Flow type Categories', 'Levels')),
...
dt.bymhi.cl2 %>%
  ...
  stat_stratum(geom = "text",
               aes(label = ifelse(as.numeric(total) >= 10, layers, NA)),
               decreasing = TRUE) +
  ...
  ggrepel::geom_text_repel(
    decreasing = TRUE,
    aes(label = ifelse(as.numeric(total) < 10, as.character(layers), NA) ),
    # segment.square  = F,
    # segment.inflect = T,
    segment.size    = 0.3,
    segment.alpha   = .7,
...
getwd()
f <- paste0('FlowType_scale_pairs', today, '-1.png')
